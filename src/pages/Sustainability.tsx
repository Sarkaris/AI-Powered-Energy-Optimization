...
import { TreePine, Award, TrendingUp, Users, Building, Download, Share } from 'lucide-react';
...
import { SustainabilityReportTemplate } from '../components/reports/SustainabilityReportTemplate';
...
export const Sustainability: React.FC = () => {
  ...
  // ✅ 2. Create a ref for the report template
  ...
  // ✅ 3. Create the data object for the report
  const esgDataForReport: ESGReportData = {
    companyName: 'EcoFlow Energy Solutions',
    reportPeriod: 'Q4 2024',
    generatedAt: new Date().toISOString(),
    environmental: mockESGMetrics.environmental,
    social: mockESGMetrics.social,
    governance: mockESGMetrics.governance,
  ...
  const generateReport = async () => {
    ...
    // Let the UI update to show the loading spinner
    ...
    try {
      ...
      if (reportElement) {
        ...
        pdf.save(`EcoFlow_Sustainability_Report_${new Date().toISOString().split('T')[0]}.pdf`);
      }
    } catch (error) {
  ...
  const shareReport = async () => {
    // This function can remain the same or be adapted as needed
    setIsSharing(true);
    try {
      await reportingService.createESGSummaryCard(esgDataForReport);
    } catch (error) {
  ...
  return (
    <>
      {/* ✅ 4. Render the report template off-screen */}
      <SustainabilityReportTemplate
        reportRef={reportTemplateRef}
        reportData={esgDataForReport}
      />
