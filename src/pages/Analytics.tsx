...
export const Analytics: React.FC = () => {
    ...
    const exportData = async (format: 'pdf' | 'excel' | 'csv' | 'save') => {
        ...
        try {
            const reportData: ReportData = {
                ...
                metrics: [
                    ...
                    { label: 'Average per Hour', value: (energyData.reduce((sum, d) => sum + d.consumption, 0) / energyData.length).toFixed(2), unit: 'kWh' },
                    { label: 'Peak Usage', value: Math.max(...energyData.map(d => d.consumption)).toFixed(1), unit: 'kWh' },
                    { label: 'Total Cost', value: energyData.reduce((sum, d) => sum + d.cost, 0).toFixed(2), unit: 'USD', trend: -3.1 },
            ...
            switch (format) {
                case 'pdf': await reportingService.exportToPDF(reportData); break;
            ...
            console.log(`${format.toUpperCase()} report action completed successfully!`);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                {/* ✅ Consolidated Export Buttons into a Dropdown */}
                <div className="relative" ref={exportMenuRef}>
                    <button
                        onClick={() => setIsExportMenuOpen(!isExportMenuOpen)}
                        className="flex items-center justify-center w-full md:w-auto space-x-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50"
                    >
                    ...
                    {isExportMenuOpen && (
                        <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border dark:border-gray-700 z-10">
                            <button onClick={() => exportData('save')} disabled={!!isExporting} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-t-lg">
                            ...
                            <button onClick={() => exportData('pdf')} disabled={!!isExporting} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <button onClick={() => exportData('excel')} disabled={!!isExporting} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <button onClick={() => exportData('csv')} disabled={!!isExporting} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-b-lg">
            ...
            {/* Controls - Already responsive, no changes needed */}
            ...
            {/* Main Chart & Predictions Panel - Already Responsive */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                    <Chart
                        ...
                        title={`${selectedMetric === 'consumption' ? 'Energy Consumption' : selectedMetric === 'cost' ? 'Cost Analysis' : 'CO₂ Emissions'} - ${timeFrame}`}
                        color={selectedMetric === 'consumption' ? '#10b981' : selectedMetric === 'cost' ? '#3b82f6' : '#f59e0b'}
                        height={320}
                    />
                </div>
